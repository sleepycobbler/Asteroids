...
void passiveMousePosCallback(int x, int y)
{
	Interface ui;
	ui.mouseMove(x, y);
}
...
* Either set the up or down event for a given click
*   INPUT   key     which key is pressed
*           fDown   down or brown
****************************************************************/
void Interface::mouseEvent(int button, int state)
{
	switch (button)
	{
...
void Interface::mouseEvent()
{
	isLeftClick = false;
	isRightClick = false;
}

void lClickDownCallback(int button, int x, int y)
{
	// Even though this is a local variable, all the members are static
	// so we are actually getting the same version as in the constructor.
	Interface ui;
	ui.mouseEvent(button, true /*fDown*/);
}

void lClickUpCallback(int button, int x, int y)
{
	// Even though this is a local variable, all the members are static
	// so we are actually getting the same version as in the constructor.
	Interface ui;
...
void Interface::mouseMove(int x, int y)
{
	mouseX = x;
	mouseY = y;
}
...
bool         Interface::isLeftClick = false;
bool         Interface::isRightClick = false;
int          Interface::mouseX = 0;
int          Interface::mouseY = 0;
...
void Interface::initialize(int argc, char ** argv, const char * title, Point topLeft, Point bottomRight)
{
	...
	glutPassiveMotionFunc(passiveMousePosCallback);
